// Path: app/(public)/blog/category/[category]/page.tsx
...
export async function generateStaticParams() {
  ...
  for (const post of posts) {
    if (post.category) {
      categories.add(slugify(post.category));
    }
  }
...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  const matchingPosts = posts.filter(
    (p) => p.category && slugify(p.category) === decodedCategory
  );

  const displayName = matchingPosts[0]?.category
    || decodedCategory.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
  const count = matchingPosts.length;

  return {
    title: `${displayName} — Doxxy Blog`,
    description: `Read ${count} article${count !== 1 ? "s" : ""} in the ${displayName} category on the Doxxy Blog. Expert insights on healthcare practice management.`,
    alternates: {
      canonical: `/blog/category/${decodedCategory}`,
    },
    openGraph: {
      title: `${displayName} — Doxxy Blog`,
      description: `${count} article${count !== 1 ? "s" : ""} in the ${displayName} category`,
    },
  };
}
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const matchingPosts = posts
    .filter((p) => p.category && slugify(p.category) === decodedCategory)
    .sort((a, b) => new Date(b.publishDate).getTime() - new Date(a.publishDate).getTime());

  const displayName =
    matchingPosts[0]?.category ||
    decodedCategory.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
  const count = matchingPosts.length;
  ...
  const collectionPageSchema = {
    ...
    description: `Blog posts in the ${displayName} category`,
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-950">
      ...
      <section className="relative overflow-hidden pt-20 pb-16 px-6 lg:px-8">
        ...
        <div className="max-w-7xl mx-auto text-center">
          <span className="inline-block px-4 py-1.5 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-sm font-medium rounded-full mb-6">
            Category
          </span>
      ...
      {/* CTA Section */}
      <SignupCTA
        heading={`Enjoying our ${displayName} articles? See Doxxy in Action.`}
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: displayName, url: `${APP_URL}/blog/category/${decodedCategory}` },
        ]}
      />
    </div>
  );
