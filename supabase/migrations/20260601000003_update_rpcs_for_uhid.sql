-- Update RPCs to reference uhid instead of deprecated medical_id column
-- Must DROP first since RETURNS TABLE signature changes
    ...
    SELECT
        ...
        (SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json)
         FROM (
            ...
            WHERE a.clinic_id = _clinic_id AND a.doctor_id = doc_id
            GROUP BY p.id, p.name, p.email, p.phone, p.gender, p.age, p.uhid, p.address, p.clinic_id, p.created_at
