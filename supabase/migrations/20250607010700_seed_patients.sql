-- Insert seed patients for testing
-- Note: Replace 'YOUR_CLINIC_ID' with your actual clinic ID

-- Get the clinic ID dynamically (replace with your actual clinic name or use the ID directly)
DO $$
DECLARE
    clinic_uuid UUID;
BEGIN
    ...
    ON CONFLICT (medical_id, clinic_id) DO NOTHING; -- Prevent duplicates if medical_id is unique per clinic
