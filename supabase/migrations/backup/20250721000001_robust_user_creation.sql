-- Enhanced robust user creation that handles errors gracefully
-- This migration replaces the previous handle_new_user function with a more robust version
...
BEGIN
    ...
    IF clinic_id_from_metadata IS NOT NULL AND role_from_metadata IS NOT NULL THEN
        BEGIN
            ...
            IF role_from_metadata = 'doctor' THEN
                ...
                EXCEPTION WHEN OTHERS THEN
                    RAISE NOTICE 'Error creating doctor profile: %', SQLERRM;
                END;
            END IF;
    ...
    -- Final success log
    RAISE NOTICE 'User processing completed: % | Profile: % | Membership: %', 
                 NEW.email, profile_created, membership_created;
    
    RETURN NEW;
    
EXCEPTION WHEN OTHERS THEN
    -- This is the absolute last resort - should never happen
    RAISE WARNING 'CRITICAL: Unexpected error in handle_new_user for %: %', 
                  COALESCE(NEW.email, NEW.id::text), SQLERRM;
    RETURN NEW;
END;
